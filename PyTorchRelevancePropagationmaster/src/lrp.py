...
from PyTorchRelevancePropagationmaster.src.utils import layers_lookup
...
class LRPModel(nn.Module):
    ...
    def _get_layer_operations(self) -> torch.nn.ModuleList:
        ...
        # Parse VGG-16
        for layer in self.model._AtariNet__features:
            layers.append(layer)

        # layers.append(self.model._AtariNet__head)
        layers.append(torch.nn.Flatten(start_dim=1))

        for layer in self.model._AtariNet__head:
            layers.append(layer)

        return layers

    def forward(self, x: torch.ByteTensor) -> torch.ByteTensor:
